const CACHE_NAME='travelmate-v21-stable-20260722';
...
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('travelmate-')&&k!==CACHE_NAME).map(k=>caches.delete(k)))))});
